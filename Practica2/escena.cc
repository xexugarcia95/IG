...
void Escena::draw_objects() {
	...
	tetraedro tetra(getTamanio());
	cubo cu(getTamanio());
	piramide pi(getTamanio());
	model_ply ply = model_ply("beethoven.ply");
	peon p;
	revolucion re(p.getVertices(),getSegmentos(),getTapas(),360);
	model_ply corazon("modelos_ply/heart.ply");
	model_ply cilin("cilindro.ply");
	revolucion revo(cilin.getVertices(),getSegmentos(),getTapas(),360);

	if(getType()==1){
		
		tetra.dibuja(getMode());
	}else if(getType()==2){
